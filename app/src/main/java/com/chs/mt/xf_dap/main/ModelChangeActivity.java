...
public class ModelChangeActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.chs_model);
        mcontext = this;
        // 首先获取到意图对象
    ...
    private void FlashPageUI() {
        for (int i = 0; i <MacCfg.ModelMax ; i++) {
            imageView[i].setVisibility(View.INVISIBLE);
        }
        imageView[getmodel(inputsourceIndex)].setVisibility(View.VISIBLE);
    }
    ...
    private void initClick() {
        for (int i = 0; i <MacCfg.ModelMax ; i++) {
            linearLayout[i].setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    inputsourceIndex=setModel((int)v.getTag());
                    FlashPageUI();
                }
            });
        }
        back_title_itemView.btn_sure.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                DataStruct.RcvDeviceData.SYS.Proc_Amp_Mode=inputsourceIndex;
                finish();
    ...
    private View addView1(int index) {
        ...
        txt_band.setText(DataStruct.CurMacMode.inputsource.Model_Name[index]);
     ...
     * 获取音源显示为哪个
     * */
    private int getmodel(int index){
        int val=0;
        switch (index){
            case 0:
                val= 0;
     ...
     * 设置音源下发哪个
     * */
    private int setModel(int index){
        int val=0;
        switch (index){
            case 0:
                val= 0;
