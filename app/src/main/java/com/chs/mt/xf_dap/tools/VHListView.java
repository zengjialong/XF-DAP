 ...
 * Some terminology:
 *
 *     chs_index    - chs_index of the items that are currently visible
 *     position - chs_index of the items in the cursor
 *
 ...
 */
public class VHListView extends AdapterView<ListAdapter> implements
    ...
    public interface OnScrollListener {
         ...
         */
        public void onScrollStateChanged(VHListView view, int scrollState);

        /**
         * Callback method to be invoked when the list or grid has been scrolled. This will be
         * called after the scroll has completed
         * @param view The view whose scroll state is being reported
         * @param firstVisibleItem the chs_index of the first visible cell (ignore if
         *        visibleItemCount == 0)
         * @param visibleItemCount the number of visible cells
         * @param totalItemCount the number of items in the list adaptor
         */
        public void onScroll(VHListView view, int firstVisibleItem, int visibleItemCount,
                int totalItemCount);
    }
     ...
     *
     * @see VHListView.RecycleBin
     * @see VHListView#setRecyclerListener(VHListView.RecyclerListener)
     */
    ...
    public VHListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VHListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    @SuppressLint("NewApi") public long[] getCheckedItemIds() {
        if (mChoiceMode.compareTo(ChoiceMode.NONE) == 0 ||
    ...
    @SuppressLint("NewApi") public void setItemChecked(int position, boolean value) {
        if (mChoiceMode.compareTo(ChoiceMode.NONE) == 0) {
            return;
        }
     ...
     */
    @SuppressLint("NewApi") public void clearChoices() {
    ...
    @SuppressLint("NewApi") public void setChoiceMode(ChoiceMode choiceMode) {
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        if (!hasWindowFocus) {
            ...
            if (touchMode != mLastTouchMode && mLastTouchMode != TOUCH_MODE_UNKNOWN) {
                // If we come chs_back in trackball mode, we bring the selection chs_back
                if (touchMode == TOUCH_MODE_OFF) {
                    // This will trigger a layout
                    resurrectSelection();

                // If we come chs_back in touch mode, then we want to hide the selector
                } else {
                    hideSelector();
                    mLayoutMode = LAYOUT_NORMAL;
                    layoutChildren();
    ...
    public boolean showContextMenuForChild(View originalView) {
        ...
        if (longPressPosition >= 0) {
            ...
            if (listener != null) {
                handled = listener.onItemLongClick(VHListView.this, originalView,
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE: {
            ...
            if (index < 0) {
                Log.e(LOGTAG, "onInterceptTouchEvent could not find chs_pointer with id " +
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(VHListView.class.getName());
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        info.setClassName(VHListView.class.getName());
    ...
    /**
     * Scrolls up or chs_down by the number of items currently present on screen.
     *
    ...
    private void handleNewSelectionChange(View selectedView, int direction, int newSelectedPosition,
        ...
        // Whether or not we are moving chs_down/right or up/left, we want to preserve the
        // top/left of whatever view is at the start:
        // - moving chs_down/right: the view that had selection
     ...
     * @param childIndex The view group chs_index of the child.
    ...
    /**
     * Handle an arrow scroll going up or chs_down.  Take into account whether items are selectable,
    ...
    private boolean arrowScrollImpl(int direction) {
        ...
        if (nextSelectedPosition == INVALID_POSITION && selectedView != null
            ...
            // unhandled key events bring chs_back the item we just scrolled off)
            mResurrectToPosition = INVALID_POSITION;
        }
    ...
    private void layoutChildren() {
        ...
        try {
            ...
            // Take focus chs_back to us temporarily to avoid the eventual
            // call to clear focus when removing the focused child below
            // from messing things up when ViewAncestor assigns focus chs_back
            // to someone else.
    ...
    @SuppressLint("NewApi") void confirmCheckedPositionsById() {
    ...
    private void handleDataChanged() {
        ...
        if (itemCount > 0) {
            ...
            if (!isInTouchMode()) {
                ...
                // Make sure we select something selectable -- first look chs_down
                selectablePos = lookForSelectablePosition(newPos, true);
                ...
                if (selectablePos >= 0) {
                    ...
                    // Looking chs_down didn't work -- try looking up
    ...
    private int getChildWidthMeasureSpec(LayoutParams lp) {
        if (!mIsVertical && lp.width == android.view.ViewGroup.LayoutParams.WRAP_CONTENT) {
    ...
    private int getChildHeightMeasureSpec(LayoutParams lp) {
        if (mIsVertical && lp.height == android.view.ViewGroup.LayoutParams.WRAP_CONTENT) {
            return MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
        } else if (!mIsVertical) {
    ...
    private void setupChild(View child, int position, int top, int left,
        ...
        if (needToMeasure) {
            final int childRight = childLeft + w;
            final int childBottom = childTop + h;
//            child.layout(childLeft, childTop, childRight, childBottom); Motinlu
        } else {
    ...
    private void correctTooHigh(int childCount) {
        ...
        if (endOffset > 0 && (mFirstPosition > 0 || firstStart < start))  {
            if (mFirstPosition == 0) {
                // Don't pull the top too far chs_down
                endOffset = Math.min(endOffset, start - firstStart);
            }

            // Move everything chs_down
    ...
    private void correctTooLow(int childCount) {
        ...
        // bottom of the list to be pushed chs_down.
    ...
    private int findSyncPosition() {
        ...
        // True if we should move chs_down on the next iteration
        boolean next = false;
        ...
        while (SystemClock.uptimeMillis() <= endTime) {
            ...
            if (hitFirst || (next && !hitLast)) {
                // Either we hit the top, or we are trying to move chs_down
                last++;
                seed = last;
    ...
    private boolean performLongPress(final View child,
        ...
        OnItemLongClickListener listener = getOnItemLongClickListener();
        if (listener != null) {
            handled = listener.onItemLongClick(VHListView.this, child,
                    longPressPosition, longPressId);
        }

        if (!handled) {
            mContextMenuInfo = createContextMenuInfo(child, longPressPosition, longPressId);
            handled = super.showContextMenuForChild(VHListView.this);
        }
        ...
        public LayoutParams(int width, int height) {
            ...
            if (this.width == MATCH_PARENT) {
                Log.w(LOGTAG, "Constructing LayoutParams with width FILL_PARENT " +
                        ...
                        "Falling chs_back to WRAP_CONTENT");
    ...
    private class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            if (VHListView.this.mHasStableIds && mInstanceState != null
                    && mOldItemCount == 0 && mItemCount > 0) {
                VHListView.this.onRestoreInstanceState(mInstanceState);
                mInstanceState = null;
            } else {
                rememberSyncState();
            }
        ...
        public void onInvalidated() {
            ...
            if (VHListView.this.mHasStableIds) {
                // Remember the current state for the case where our hosting activity is being
                // stopped and later restarted
                mInstanceState = VHListView.this.onSaveInstanceState();
            }
    ...
    static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from {@link VHListView#onSaveInstanceState()}
         */
         ...
         */
        @SuppressLint("NewApi") private SavedState(Parcel in) {
            super(in);
    ...
    private static class ArrowScrollFocusResult {
        ...
        /**
         * How {@link VHListView#arrowScrollFocused} returns its values.
         */
